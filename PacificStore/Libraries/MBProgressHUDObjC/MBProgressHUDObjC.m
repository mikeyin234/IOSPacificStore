...
#define MBMainThreadAssert() NSAssert([NSThread isMainThread], @"MBProgressHUDObjC needs to be accessed on the main thread.");
...
@interface MBProgressHUDObjCRoundedButton : UIButton
@end


@implementation MBProgressHUDObjC

#pragma mark - Class methods

+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    MBProgressHUDObjC *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    MBProgressHUDObjC *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (MBProgressHUDObjC *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            MBProgressHUDObjC *hud = (MBProgressHUDObjC *)subview;
...
- (void)commonInit {
    ...
    _animationType = MBProgressHUDObjCAnimationFade;
    _mode = MBProgressHUDObjCModeIndeterminate;
    _margin = 20.0f;
...
- (void)animateIn:(BOOL)animatingIn withType:(MBProgressHUDObjCAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == MBProgressHUDObjCAnimationZoom) {
        type = animatingIn ? MBProgressHUDObjCAnimationZoomIn : MBProgressHUDObjCAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == MBProgressHUDObjCAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == MBProgressHUDObjCAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == MBProgressHUDObjCAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == MBProgressHUDObjCAnimationZoomOut) {
            bezelView.transform = small;
        }
...
- (void)done {
    ...
    MBProgressHUDObjCCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<MBProgressHUDObjCDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    backgroundView.style = MBProgressHUDObjCBackgroundStyleSolidColor;
    ...
    UIButton *button = [MBProgressHUDObjCRoundedButton buttonWithType:UIButtonTypeCustom];
...
- (void)updateIndicators {
    ...
    MBProgressHUDObjCMode mode = self.mode;
    if (mode == MBProgressHUDObjCModeIndeterminate) {
    ...
    else if (mode == MBProgressHUDObjCModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[MBBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == MBProgressHUDObjCModeDeterminate || mode == MBProgressHUDObjCModeAnnularDeterminate) {
        if (!isRoundIndicator) {
        ...
        if (mode == MBProgressHUDObjCModeAnnularDeterminate) {
            [(MBRoundProgressView *)indicator setAnnular:YES];
        }
    } 
    else if (mode == MBProgressHUDObjCModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == MBProgressHUDObjCModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
- (void)updateViewsForColor:(UIColor *)color {
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[MBProgressHUDObjC class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[MBProgressHUDObjC class]]];
#endif
...
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [MBRoundProgressView appearanceWhenContainedIn:[MBProgressHUDObjC class], nil];
#else
        appearance = [MBRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[MBProgressHUDObjC class]]];
#endif
...
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [MBBarProgressView appearanceWhenContainedIn:[MBProgressHUDObjC class], nil];
#else
        appearance = [MBBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[MBProgressHUDObjC class]]];
#endif
...
- (void)setMode:(MBProgressHUDObjCMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == MBProgressHUDObjCModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_7_0) {
            _style = MBProgressHUDObjCBackgroundStyleBlur;
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
            ...
            _style = MBProgressHUDObjCBackgroundStyleSolidColor;
...
- (void)setStyle:(MBProgressHUDObjCBackgroundStyle)style {
    if (style == MBProgressHUDObjCBackgroundStyleBlur && kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNumber_iOS_7_0) {
        style = MBProgressHUDObjCBackgroundStyleSolidColor;
    }
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
...
- (void)updateForBackgroundStyle {
    MBProgressHUDObjCBackgroundStyle style = self.style;
    if (style == MBProgressHUDObjCBackgroundStyleBlur) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == MBProgressHUDObjCBackgroundStyleBlur) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
            self.backgroundColor = self.color;
        } else {
#if !TARGET_OS_TV
...
@implementation MBProgressHUDObjCRoundedButton
